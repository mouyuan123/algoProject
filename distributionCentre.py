...
def showPath(location, des, API_key):
    ...
    gmap1 = gmplot.GoogleMapPlotter(float(location.coor.split(", ")[0]), float(location.coor.split(", ")[1]), zoom=13,
                                    apikey=API_key)
    origin = gmaps.reverse_geocode((location.coor.split(", ")[0], location.coor.split(", ")[1]))
    originAdd = origin[0]["formatted_address"]
    print("Centre Address: ", originAdd)
    ...
    for destination in des:
        gmap1.directions(origin=(float(location.coor.split(", ")[0]), float(location.coor.split(", ")[1])),
                         ...
                         color='red')
        destination1 = gmaps.reverse_geocode((destination.split(", ")[0], destination.split(", ")[1]))
        desAdd = destination1[0]["formatted_address"]
        if originAdd != desAdd:
